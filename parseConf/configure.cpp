...
int32_t ConfigSet::GetIntVal( const char * sectname, const char * key, int32_t default_val ) const
{
...
int ConfigSet::SetIntVal( const char * sectname, const char * key, int32_t value )
{
	//ASSERT_RET( ( sectname != NULL && key != NULL ), -1 );
...
double ConfigSet::GetInt64Val( const char * sectname, const char * key, double default_val ) const
{
	...
	val = strtoll( value, &endptr, 0 );			// accept base of 8, 10 or 16
	if ( val == LONG_LONG_MIN || val == LONG_LONG_MAX ) {		// underflow or overflow
		if ( errno == ERANGE )
			return default_val;
	}
	if ( endptr == value || *endptr != '\0' )	// not an integer at all
